...
class FileStream(trio.abc.Stream):
    ...
    async def receive_some(self, n):
        return await self.wrapped.read1(n)
...
async def main():
#    con = Console()
    with open('samp.txt', mode='rb', buffering=4) as f:
        print f.readline()
    
    async with await trio.open_file('samp.txt', mode='rb', buffering=4) as f:
        async for line in f:
            print("<", line, ">")
    exit(1)
    
    
    fs = FileStream(sys.stdin)
    while True:
        c = await fs.wrapped.read1(12)
        if not c:
            print ('DONE', c, 'DONE')
            break
        print("<", c, ">")
